...
fork(void)
{
  ...
  if((np->pgdir = copyuvm(curproc->pgdir, curproc->sz, curproc->ptableindex)) == 0){
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
    return -1;
...
waitpid(int pid, int *status, int options)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      havekids = 1;
      if(p->state == ZOMBIE && (p->pid == pid || pid == 0)){ 
        // Found one.
	pidRet = p->pid;
	if (status != 0) //checks if status is null
		*status = p->exitStatus;
        kfree(p->kstack);
        p->kstack = 0;
        freevm(p->pgdir);
      ...
      else if (options == WNOHANG && p->pid == pid){
        release(&ptable.lock);
	return 0;
      }
